...
public final class SqlUtil {
     ...
     * 解析表信息（向后兼容）
     */
    private static void parseTableInfo(String sql, SqlAnalysisInfo result) {
        // 这个方法保留用于向后兼容，实际功能已经分散到各个具体的解析方法中
     ...
     * 解析 SET 字段（向后兼容）
     */
    private static void parseSetFields(String setClause, SqlAnalysisInfo result) {
        if (StringUtil.isEmpty(setClause)) {
            return;
        }
        ...
        Matcher fieldMatcher = SET_FIELD_PATTERN.matcher(setClause);
        while (fieldMatcher.find()) {
            String columnName = getFirstNonNull(fieldMatcher.group(1), fieldMatcher.group(2));
            if (columnName != null) {
                FieldCondition condition = new FieldCondition(null, columnName, FieldType.UPDATE_SET);
